...
function mapDraftStatusToDb(status = "DANG_LAP") {
  ...
  if (value === "CHO_DOI_SOAT") return "CHO_DOI_SOAT";
  return "DANG_LAP";
}
...
function matchWorkItemStatus(item, rawStatus) {
  const status = String(rawStatus || "").toUpperCase();
  if (!status || status === "ALL") {
    return true;
  }

  const needsAdditionalPayment = toNumber(item.additionalPaymentAmount) > 0;

  if (status === "CHO_DOI_SOAT") return item.workflowStatus === "CHO_DOI_SOAT";
  if (status === "DANG_LAP") return item.workflowStatus === "DANG_LAP";
  if (status === "CAN_THU_THEM") return needsAdditionalPayment;
  if (status === "DA_CHOT") return item.workflowStatus === "DA_CHOT";
...
const AccountingReconciliationPageModel = {
  ...
  async listWorkItems(filters = {}) {
    ...
    if (filters.status) {
      items = items.filter((item) => matchWorkItemStatus(item, filters.status));
    }
    ...
    const currentLimit = Math.max(Number(filters.limit) || 10, 1);
    const from = (currentPage - 1) * currentLimit;
    const paginatedItems = items.slice(from, from + currentLimit);
  ...
  async finalize(reconciliationId) {
    ...
    let autoGeneratedDocument = null;

    if (toNumber(detail.additionalPaymentAmount) > 0) {
      const voucher = await this.createAdditionalPaymentVoucher(reconciliationId);
      autoGeneratedDocument = {
        type: "ADDITIONAL_PAYMENT",
        voucher,
      };
    } else if (toNumber(detail.refundAmount) > 0) {
      const voucher = await this.createRefundVoucher(reconciliationId, {
