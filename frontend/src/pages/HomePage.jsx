               ...
               <div key={room.id} className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden group hover:shadow-xl transition-shadow duration-300">
                   ...
                   <div className="p-6">
                      ...
                      <div className="mt-6 flex gap-3">
                          {room.status === 'ĐÃ ĐẦY' ? (
                             <div className="flex-1 bg-gray-300 text-gray-500 flex items-center justify-center py-3 rounded-xl font-bold text-[14px] cursor-not-allowed">Phòng đã đầy</div>
                          ) : (
                             <Link to={`/booking/${room.id}`} className="flex-1 bg-[#0A192F] flex items-center justify-center text-white py-3 rounded-xl font-bold text-[14px] hover:bg-[#112240] transition-colors shadow-md">Đặt cọc ngay</Link>
                          )}
