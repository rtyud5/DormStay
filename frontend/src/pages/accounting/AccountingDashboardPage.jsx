...
export default function AccountingDashboardPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const [kpiResult, invoicesResult, contractsResult] = await Promise.allSettled([
          getDashboardKPI(),
          getInvoices({
            limit: 5,
            status: "PENDING",
        ...
        if (kpiResult.status === "fulfilled") {
          setKpi({ ...EMPTY_KPI, ...(kpiResult.value.data || {}) });
        } else {
          console.error("Error loading dashboard KPI:", kpiResult.reason);
          setKpi(EMPTY_KPI);
        }

        if (invoicesResult.status === "fulfilled") {
          setRecentInvoices(invoicesResult.value.data || []);
        } else {
          console.error("Error loading recent invoices:", invoicesResult.reason);
          setRecentInvoices([]);
        }

        if (contractsResult.status === "fulfilled") {
          setOverdueContracts((contractsResult.value.data || []).slice(0, 5));
        } else {
          console.error("Error loading active contracts:", contractsResult.reason);
          setOverdueContracts([]);
        }
      } catch (error) {
  ...
  const invoiceTotal = Math.max(kpi?.invoiceStats?.total || 0, 1);
  const invoiceCompleted = kpi?.invoiceStats?.completed || 0;
  const invoiceOverdue = kpi?.invoiceStats?.overdue || 0;
  const invoicePending = kpi?.invoiceStats?.pending || 0;
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <p className="text-gray-500 font-medium">
              Báo cáo hiệu suất, dòng tiền và kiểm soát tình hình công nợ thời gian thực.
            </p>
          </div>
            ...
            <div className="space-y-6">
              <div>
                ...
                <p className="text-3xl font-black text-gray-900 leading-none">
                  {formatCurrency(kpi?.totalRevenue || 0).replace("₫", "")}{" "}
              ...
              <div className="flex items-start gap-4">
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${(kpi?.monthlyChange || 0) >= 0 ? "bg-[#eaffec] text-[#22a654]" : "bg-red-100 text-red-600"}`}
                >
                  {(kpi?.monthlyChange || 0) >= 0 ? (
                ...
                <div>
                  ...
                  <p className="text-[11px] font-medium text-gray-500 leading-relaxed">
                    Doanh thu {(kpi?.monthlyChange || 0) >= 0 ? "tăng" : "giảm"}{" "}
                    <strong className="text-gray-700">{Math.abs(kpi?.monthlyChangePercent || 0)}%</strong> so với tháng
                    trước.
                  </p>
                </div>
              </div>
        ...
        <div className="bg-white rounded-[2rem] shadow-sm border border-gray-100 flex flex-col p-2">
          ...
          <div className="px-2 pb-2">
            <AccountingInvoiceTable
              ...
              onRowClick={() => navigate(ACCOUNTING_ROUTES.INVOICES)}
              actions={[
                {
                  icon: "👁️",
                  label: "Mở danh sách",
        ...
        <div className="bg-white rounded-[2rem] shadow-sm border border-gray-100 flex flex-col p-2">
          ...
          <div className="px-2 pb-2">
            <AccountingInvoiceTable
              ...
              onRowClick={() => navigate(ACCOUNTING_ROUTES.CONTRACTS)}
              actions={[
                {
                  icon: "✏️",
                  label: "Lập phiếu",
